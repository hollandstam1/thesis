...
ggplot(lmm_aug, aes(x = moma_count_to_year, y = .resid)) +
  ...
  labs(
    title = "Linearity Assumption Satisfied",
    x = "MoMA Count to Year",
    y = "Residuals"
  )
```

There is no unexpected pattern left behind and the residuals are randomly and symmetrically scattered about the x-axis therefore linearity is satisfied. There is a linear relationship between the response and predictor variables.
